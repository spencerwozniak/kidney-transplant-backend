...
async def get_questionnaire(request: Request):
    ...
    Returns the most recent questionnaire for the device's patient
    """
    device_id = get_device_id(request)
    questionnaire = database.get_questionnaire(device_id)
...
async def submit_questionnaire(submission: QuestionnaireSubmission, request: Request):
    """
    Save questionnaire
    
    Verifies patient exists, adds ID, stores with patient_id and timestamp
